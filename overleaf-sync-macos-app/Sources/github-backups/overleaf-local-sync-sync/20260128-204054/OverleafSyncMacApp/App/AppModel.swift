...
final class AppModel: ObservableObject {
  ...
  @Published var projectDeletePrompt: ProjectDeletePrompt?
  ...
  func archiveSelectedProject() async {
    ...
    do {
      ...
      try await client.projectArchive(
        baseURL: baseURL,
  ...
  func unarchiveSelectedProject() async {
    ...
    do {
      ...
      try await client.projectUnarchive(
        baseURL: baseURL,
        projectId: project.id,
        email: email.isEmpty ? nil : email,
        password: password.isEmpty ? nil : password
  ...
  func untrashSelectedProject() async {
    ...
    do {
      ...
      try await client.projectUntrash(
        baseURL: baseURL,
        projectId: project.id,
        email: email.isEmpty ? nil : email,
        password: password.isEmpty ? nil : password
  ...
  func prepareDeleteSelectedProject() {
    ...
    let effectiveProjectId = cfg?.projectId ?? project.id
    projectDeletePrompt = ProjectDeletePrompt(
  ...
  func deleteProject(
    prompt: ProjectDeletePrompt,
    cloud: CloudProjectDeletion,
    local: LocalProjectDeletion,
    deleteInbox: Bool,
    deleteBackups: Bool,
    deleteSnapshots: Bool
  ) async {
    let pid = prompt.projectId
    ...
    func stopWatchesIfNeeded() {
      guard let linkedDir else { return }
      for w in watches where w.dir.standardizedFileURL == linkedDir.standardizedFileURL {
        w.stop()
      }
      if let ext = externalWatches.first(where: { $0.dir.standardizedFileURL == linkedDir.standardizedFileURL }) {
        stopExternalWatch(ext)
      }
    }

    do {
      // Stop watches before any destructive actions.
      if cloud != .none || local != .none {
        stopWatchesIfNeeded()
      }

      // 1) Cloud operation (if any)
      if cloud != .none {
        let client = try makeClient()
        switch cloud {
        case .none:
        ...
        case .permanentlyDelete:
          appendLog("→ project-delete \(pid)")
          try await client.projectDeletePermanently(
            baseURL: effectiveBase,
            projectId: pid,
            email: email.isEmpty ? nil : email,
            password: password.isEmpty ? nil : password
        ...
        let opResult = await Task.detached { () -> (Bool, String?) in
          do {
            ...
            case .none:
              return (true, nil)
            case .moveToTrash:
              _ = try FileManager.default.trashItem(at: linkedDir, resultingItemURL: nil)
              return (true, nil)
            case .permanentlyDelete:
              try FileManager.default.removeItem(at: linkedDir)
              return (true, nil)
            }
          } catch {
      ...
      // 3) Tool data cleanup (optional; local only)
      if deleteInbox || deleteBackups || deleteSnapshots {
        let inboxDirToRemove = deleteInbox ? inboxHostDir(baseURL: effectiveBase).appendingPathComponent(pid) : nil
        let backupsDirToRemove = deleteBackups ? backupsHostDir(baseURL: effectiveBase).appendingPathComponent(pid) : nil
        let snapshotsDirToRemove = deleteSnapshots ? snapshotsHostDir(baseURL: effectiveBase).appendingPathComponent(pid) : nil

        let removed = await Task.detached { () -> [String] in
          var removed: [String] = []
          let fm = FileManager.default

          func tryRemove(_ url: URL) -> Bool {
          ...
          if let inboxDirToRemove, tryRemove(inboxDirToRemove) { removed.append("inbox") }
          if let backupsDirToRemove, tryRemove(backupsDirToRemove) { removed.append("backups") }
          if let snapshotsDirToRemove, tryRemove(snapshotsDirToRemove) { removed.append("snapshots") }
          return removed
        }.value

        if !removed.isEmpty {
          appendLog("tool data removed: \(removed.joined(separator: ", "))")
        }
      }
  ...
  private func backupsHostDir(baseURL: String) -> URL {
    configRootURL()
      .appendingPathComponent("overleaf-sync")
  ...
  private func backupsProjectDir(baseURL: String, projectId: String) -> URL {
    backupsHostDir(baseURL: baseURL).appendingPathComponent(projectId)
  }
